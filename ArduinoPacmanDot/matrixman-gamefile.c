...
uint8_t canMove(uint8_t nextX, uint8_t nextY) {
    if (board[nextY] & (tmpone<<(31-nextX))) {
        return FALSE;
    }
    else return TRUE;
}
...
uint8_t isPixel(uint8_t x, uint8_t y) {
    if (dotTracker[y] & tmpone<<(31-x)) { return TRUE; }
    return FALSE;
}
...
void movePlayer(Player *pawn) {
    ...
    if (canMove(testX, testY)) {
        ...
        if ((pawn == &myGuy) && isPixel(pawn->x,pawn->y)) {
            dotTracker[pawn->y] &= ~(tmpone<<(31-pawn->x));  //Remove dot from the board
            score += 10;
            drawScore();
...
void setupLevel(void) {
    ...
    for (i = 2; i < 34; i++) {
        for (j = 0; j<32; j++) {
            if (board[i] & (tmpone<<(31-j))) {    //Invert the x (big endian)
    ...
    for (i = 2; i < 34; i++) {
        for (j = 0; j<32; j++) {
            if (dotTracker[i] & (tmpone<<(31-j))) {    //Invert the x (big endian)
...
void drawScore(void) {
    int8_t i; //Arduino IDE not set for c99 by default; booo!
    for (i=0; i<32; i++) {
        if (score & (tmpone<<i)) { displayPixel(31, 33-i, WHITE); }
    ...
    */
    ...
    while (programRunning)
    {
        ...
        if (myGuy.dotCount == 244) {
            ...
            //Life lost, restart this level after button press
            showDisplayStart();
